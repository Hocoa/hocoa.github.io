$(function() {
  ...
  const isTocContent =
    $("#sidebar .sidebar-toc__content").children().length > 0;
  /**
   * 當menu過多時，自動適配，避免UI錯亂
   */
  const searchWidth =
    $("#search_button").outerWidth() !== undefined
      ? $("#search_button").outerWidth()
      : 0;
  ...
  if (GLOBAL_CONFIG.isPost) {
    ...
    // mobile隱藏
    if (
      $("#mobile-sidebar-toc .sidebar-toc__content").children().length === 0
    ) {
  ...
  function openMobileSidebar(name) {
    ...
    $("#page-header.fixed.visible").css(
      "transform",
      "translate3d(-250px, 100%, 0)"
    );
  ...
  $(window).scroll(
    throttle(function(event) {
      ...
      if (currentTop > 56) {
        if (isUp) {
          if ($("#page-header").hasClass("visible"))
  ...
  if (GLOBAL_CONFIG.runtime) {
    ...
    var showDateTime = function() {
      ...
      $(".webinfo-runtime-count").text(
        daysold + " " + GLOBAL_CONFIG.runtime_unit
      );
    };
  ...
  if (
    GLOBAL_CONFIG.localSearch === undefined &&
    GLOBAL_CONFIG.algolia !== undefined
  ) {
